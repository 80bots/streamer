...
class Storage {
   ...
   */
  getFolders(type) {
    if (!Object.values(OUTPUT_TYPES).includes(type))
  ...
  getOutputFolders() {
    ...
    if (fs.existsSync(config.app.outputFolder)) {
      folders = fs
        .readdirSync(config.app.outputFolder, { withFileTypes: true })
  ...
  _getFiles(folder) {
    if (fs.existsSync(this._resolvePath(folder))) {
      return fs
        .readdirSync(this._resolvePath(folder), { withFileTypes: true })
        ...
        case OUTPUT_TYPES.JSON: {
          ...
          try {
            buff = fs.readFileSync(
              this._resolvePath(type) + "/" + this[type][folder].file
            );
    ...
    switch (type) {
      ...
      case OUTPUT_TYPES.FILES:
        ...
        return Object.values(this[type]).reduce((all, current) => {
          return all.concat(
            JSON.parse(
              fs.readFileSync(this._resolvePath(type) + "/" + current.file)
            )
          );
        }, []);
      case OUTPUT_TYPES.IMAGES:
      ...
      for (let folder in this[type]) {
        if (this[type].hasOwnProperty(folder)) {
          if (this[type][folder]?.files) {
            ...
            this[type][folder].thumbnail = fs.readFileSync(
              this._resolvePath(type) + "/" + file
            );
          }
        }
      }
    ...
    switch (type) {
      ...
      case OUTPUT_TYPES.IMAGES: {
        ...
        watcher.on("add", filePath =>
          this._onFileAdded(filePath, type, sender)
        );
        break;
      }
      ...
      case OUTPUT_TYPES.LOG.INIT: {
        watcher = watch(typePath, {
          persistent: true,
          usePolling: true,
          ignorePermissionErrors: true
        });
        watcher.on("change", (filePath, stats) =>
          this._onFileChanged(filePath, stats, type, sender)
        );
        break;
      }
      ...
      case OUTPUT_TYPES.JSON: {
        ...
        if (fs.existsSync(filePath)) {
          ...
          watcher.on("change", (watchPath, stats) =>
            this._onFileChanged(watchPath, stats, type, sender)
          );
        }
        break;
      }
      ...
      case OUTPUT_TYPES.GENERAL: {
        ...
        watcher.on("addDir", watchPath =>
          this._onFolderCreated(watchPath, sender)
        );
        break;
      }
    }
  ...
  _onFolderCreated = (fullPath, sender) => {
    try {
      ...
      const type = Object.keys(OUTPUT_TYPES).find(
        key => OUTPUT_TYPES[key] === folderName
      );
  ...
  _getDate = fileName =>
    dayjs(fileName.split(".")[0], config.app.dateFormat).format("YYYY-MM-DD");

  _size = (type, setTo) => {
    switch (type) {
      case OUTPUT_TYPES.LOG.WORK:
      case OUTPUT_TYPES.LOG.INIT:
        return typeof setTo === "number"
          ? (this.logSize = setTo)
          : this.logSize;
  ...
  _compressImages = async () =>
    new Promise(resolve => {
      ...
      if (!Object.keys(this[OUTPUT_TYPES.IMAGES]).length)
        this._getImageFolders(OUTPUT_TYPES.IMAGES);
      ...
      try {
        ...
        this[OUTPUT_TYPES.IMAGES][key].files.forEach(item => {
          archive.append(
            fs.readFileSync(
              this._resolvePath(OUTPUT_TYPES.IMAGES) + "/" + item
            ),
            { name: item }
          );
        });
          ...
          case OUTPUT_TYPES.IMAGES: {
            ...
            for (let folder in this[type]) {
              if (this[type].hasOwnProperty(folder)) {
                ...
                for (let idx in files) {
                  if (
                    files.hasOwnProperty(idx) &&
                    !this.syncedFiles[type].includes(files[idx])
                  ) {
                    const key = S3_PATH[type] + "/" + folder + "/" + files[idx];
                    const buffer = fs.readFileSync(
                      this._resolvePath(type) + "/" + files[idx]
                    );
            ...
            for (let folder in this[type]) {
              if (this[type].hasOwnProperty(folder) && this[type][folder]) {
                ...
                const key =
                  S3_PATH[type] +
                  "/" +
                  dayjs(folder).format("YYYY-MM-DD") +
                  "/" +
                  file;
                const buffer = fs.readFileSync(
                  this._resolvePath(type) + "/" + file
