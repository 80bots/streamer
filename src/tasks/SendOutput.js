...
class SendOutput {
  ...
  listeners = {
    ...
    [SendOutput.EVENTS.GET_FULL_OUTPUT]: (query = {}) =>
      this._getFullOutput(query)
  };
  ...
  closeWatchers() {
    this.typeWatcher &&
      storage._closeWatcher(OUTPUT_TYPES.GENERAL, this.typeWatcher);
  ...
  _getAvailableOutputs() {
    this.typeWatcher &&
      storage._closeWatcher(OUTPUT_TYPES.GENERAL, this.typeWatcher);
    this.typeWatcher = storage._initWatcher(
  ...
  _getOutput({ folder, type, ...rest }) {
    ...
    this.socket.emit(
      SendOutput.MESSAGES.OUTPUT,
      storage.getOutputData({ folder, type, ...rest })
    );
  }
  ...
  _getFullOutput({ type }) {
    storage
      .getFullOutput(type)
      .then(data => this.socket.emit(SendOutput.MESSAGES.FULL, data));
  }

  _getFolders({ type }) {
    this.folderWatcher &&
      storage._closeWatcher(OUTPUT_TYPES.JSON, this.folderWatcher);
  ...
  _onOutput = (chunk, type) => {
    if (type === OUTPUT_TYPES.JSON) {
      try {
        ...
        logger.error(
          `Broken chunk: ${chunk.toString()}; length: ${chunk.length}`
        );
      }
    } else {
